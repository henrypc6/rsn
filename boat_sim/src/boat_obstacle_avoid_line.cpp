...
int move(double tx, double ty,bool stop,bool unit_step)
{
  ...
  while(fabs(current_x-tx)>ERR_CIRCLE || fabs(current_y-ty)>ERR_CIRCLE && ros::ok())
	...
	ca=fabs(ca);
      ...
      if (stop)
	{
	  if (echo_dist>AVOID_DIST && echo_dist<29)
	    {
	      usleep(1e5);
...
// Subscribe
  ...
  move(start_x,start_y,true,false); //only move thats not blocking
  ...
  while(continueon){
    ...
    //LCM.set_waypoint();
	  ...
	  double line_theta = ls_line.get_theta();
	  double line_x = 0;
	  double line_y = ls_line.get_offset();
	  double alpha = atan2(current_y-line_y,current_x-line_x);
	  double beta = fmod((alpha-line_theta) + 4*M_PI,2*M_PI);
	  if (beta>M_PI) line_theta = fmod(line_theta + M_PI,2*M_PI);
	  
	  printf("line theta %f\n",line_theta);
	  LCM.set_line_theta(line_theta);
	  ...
	  //continueon = draw_circle((-1)*LCM.get_dir()); //-1 because get_dir is the *next* dir
	  continueon = draw_circle(LCM.get_dir()); //-1 because get_dir is the *next* dir
	 
	}

  printf("done\n");
  move(current_x,target_y+15,false,false); 
